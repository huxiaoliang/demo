...
unset https_proxy
 ...
 kubeadm init --apiserver-advertise-address=172.30.0.81  --pod-network-cidr=10.4.0.0/16  --service-cidr=10.5.0.0/16 --kubernetes-version v1.19.7 

  sudo cp -f /etc/kubernetes/admin.conf $HOME/.kube/config
  
  yq -i eval \
          '.clusters[].cluster.server |= sub("172.30.0.81", "172.30.0.81") | .contexts[].name = "cluster-c" | .current-context = "cluster-c"' \
          $HOME/.kube/config

sleep 60 
kubectl label node vm-0-81-ubuntu  submariner.io/gateway=true
...
DATASTORE_TYPE=kubernetes calicoctl create -f /root/cluster3.yaml